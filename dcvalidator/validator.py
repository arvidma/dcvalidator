...
class Validator:
    ...
    def __autosearch_github__(self, project, path):
        print("https://api.github.com/search/code?q={}+in:path+org:{}&type=Code".format(path,
                                                                                        project))
        f = urllib.request.urlopen(
            "https://api.github.com/search/code?q={}+in:path+org:{}&type=Code".format(path,
                                                                                      project))
        ...
        for result in results:
            if result["name"].endswith("docker-compose.yml") or (
                    result["name"].endswith(".yml") and "docker-compose" in result["name"]):
        ...
        if type(itterable) == type(dict()):
            ...
            for index in range(1, len(temp_itterable) - 1):
                if priority[temp_itterable[index]] < priority[temp_itterable[index + 1]]:
                    self.__log_writer__('Top level property warning:' + str(
                        temp_itterable[index]) + 'better not be before' + str(
                        temp_itterable[index + 1]))
                if priority[temp_itterable[index - 1]] < priority[temp_itterable[index]]:
                    self.__log_writer__('Top level property warning:' + str(
                        temp_itterable[index]) + 'better not be after' + str(
                        temp_itterable[index - 1]))
        if type(itterable) == type(list()):
            for index in range(1, len(itterable) - 1):
                if priority[itterable[index][0]] < priority[itterable[index + 1][0]]:
                    self.__log_writer__('Top level property warning:' + str(
                        itterable[index][0]) + 'better not be before' + str(
                        itterable[index + 1][0]))
                if priority[itterable[index - 1][0]] < priority[itterable[index][0]]:
                    self.__log_writer__('Top level property warning:' + str(
                        itterable[index][0]) + 'better not be after' + str(itterable[index - 1][0]))
                ...
                if len(tag_list_similarity) > 0:
                    for tag in tag_list_similarity:
                        if len(tag_list_similarity[tag]) > 0:
                            err_message += "I can not find '" + str(
                                tag) + "' tag. Maybe you can use: \n"
                            for item in tag_list_similarity[tag]:
                ...
                for configname in c['configs']:
                    ...
                    if 'file' in c['configs'][configname]:
                        if not os.path.exists(c['configs'][configname]['file']):
                            ...
                            self.__log_writer__(
                                "Check the file {}".format(c['configs'][configname]['file']))
                            self.__log_writer__("=============================================")
                ...
                for service in c["services"]:
                    ...
                    if 'Container name' in labelArray:
                        if not "container_name" in c["services"][service]:
                            ...
                            self.__log_writer__(
                                "Duplicate container name: " + c["services"][service][
                                    "container_name"])
                        ...
                        if type(c["services"][service]["volumes"]) == type(list()):
                            for volume in c["services"][service]["volumes"]:
                                if type(volume) == type(""):
                                    ...
                                    if 'Typing mistakes' in labelArray:
                                        err_message = ""
                                        tag_list_similarity = self.__typomistake__(volume,
                                                                                   'volumes')
                                        if len(tag_list_similarity) > 0:
                                            for tag in tag_list_similarity:
                                                if len(tag_list_similarity[tag]) > 0:
                                                    err_message += "I can not find '" + str(
                                                        tag) + "' tag under '" + service + "' service. Maybe you can use: \n"
                                                    for item in tag_list_similarity[tag]:
                                                        err_message += str(item[1]) + '\n'
                                        if len(err_message) > 0:
                                            self.__log_writer__(
                                    ...
                                    if 'type' in volume:
                                        if volume['type'] not in 'volume bind tmpfs npipe':
                                            ...
                                            self.__log_writer__(
                                                "Wrong type {} for volume \nVolume types are: volume, bind, tmpfs, npipe".format(
                                                    volume['type']))
                                            self.__log_writer__(
                                                "=============================================")
                                    if 'source' in volume:
                        ...
                        for port in c["services"][service]["ports"]:
                            if type(port) == type(""):
                                try:
                                    ...
                                    self.__log_writer__(
                                        "Tip: It's better to use the HOST:CONTAINER structure")
                                else:
                                    port_host = port_temp[0]
                                    if port_host in cacheports:
                                        self.__log_writer__(
                                            "Duplicate ports in service " + service + " port " + str(
                                                port_host))
                    ...
                    if 'Typing mistakes' in labelArray:
                        err_message = ""
                        tag_list_similarity = self.__typomistake__(c["services"][service],
                                                                   'service')
                        if len(tag_list_similarity) > 0:
                            for tag in tag_list_similarity:
                                if len(tag_list_similarity[tag]) > 0:
                                ...
                                for port in expose:
                                    if 1 < port < 65536:
                                        if port not in cacheexpose:
                                            ...
                                            self.__log_writer__("Under service: {}".format(service))
                                            self.__log_writer__(
                                                "Duplicate port {} exposed!".format(port))
                                            self.__log_writer__(
                                                "=============================================")
                                    else:
                                        self.__log_writer__(
                                        ...
                                        self.__log_writer__("Under service: {}".format(service))
                                        self.__log_writer__(
                                            "The port {} that exposed is not appropriate!".format(
                                                port))
                                        self.__log_writer__(
                                            "=============================================")
                            else:
                    ...
                    if 'depends_on' in c["services"][service]:
                        for denpendecy in c["services"][service]['depends_on']:
                            if denpendecy not in cacheService:
                                ...
                                self.__log_writer__("Under service: {}".format(service))
                                self.__log_writer__(
                                    "Wrong dependency! There is no such service with name of {}".format(
                                        denpendecy))
                                self.__log_writer__("=============================================")
                        ...
                        if type(build) == type(""):
                            ...
                            if 'context' in build:
                                if os.path.exists(build['context']):
                                    ...
                                    self.__log_writer__("Under service: {}".format(service))
                                    self.__log_writer__(
                                        "Check build context directory " + str(build['context']))
                                    self.__log_writer__(
                                        "=============================================")
                            if 'dockerfile' in build:
                                ...
                                if not os.path.exists(dockerfilepath):
                                    ...
                                    self.__log_writer__("Under service: {}".format(service))
                                    self.__log_writer__(
                                        "Check build dockerfile " + str(dockerfilepath))
                                    self.__log_writer__(
                                        "=============================================")
                        ...
                        if type(configs) == type(list()):
                            if type(configs[0]) == type(""):
                                ...
                                for config in configs:
                                    if 'Typing mistakes' in labelArray:
                                        err_message = ""
                                        tag_list_similarity = self.__typomistake__(configs,
                                                                                   'configs')
                                        if len(tag_list_similarity) > 0:
                                            for tag in tag_list_similarity:
                                                if len(tag_list_similarity[tag]) > 0:
                                    ...
                                    if 'source' in config:
                                        if config['source'] not in cacheconfigs:
                                            ...
                                            self.__log_writer__("Under service: {}".format(service))
                                            self.__log_writer__(
                                                "I can not find 'source' {} in config ".format(
                                                    str(config)))
                                            self.__log_writer__(
                                                "=============================================")
    ...
    def validator(self, autosearch, filebasedlist, urlbased, eventing, filebased=None,
        ...
        for label in alltags:
            self.__log_writer__("- {}: {} ({:.1f}% coverage)".format(label, alltags[label],
                                                                     100 * alltags[
                                                                         label] / numservices))
